"""
MCP LangGraph Agent - Model Context Protocol을 지원하는 확장된 LangGraph Agent (개선된 버전)
"""
...
class MCPLangGraphAgent(BaseAgent):
    """MCP를 지원하는 확장된 LangGraph Agent 클래스 (개선된 버전)"""
    ...
    def input_processor(self, state: MCPAgentState) -> MCPAgentState:
        """입력 처리 노드"""
    ...
    def llm_call(self, state: MCPAgentState) -> MCPAgentState:
...
필요한 경우 적절한 도구를 사용하여 사용자의 질문에 답변하세요."""
    ...
    async def tool_executor(self, state: MCPAgentState) -> MCPAgentState:
        """Tool 실행 노드 (MCP 지원)"""
    ...
    def should_continue(self, state: MCPAgentState) -> Literal["tool_executor", "response_formatter"]:
        """Tool 실행 여부 결정"""
        tool_calls = state.get("tool_calls", [])
    ...
    def response_formatter(self, state: MCPAgentState) -> MCPAgentState:
        """응답 포맷팅 노드"""
        print("📝 응답 포맷팅 중...")
        ...
        if tool_results:
            formatted_response = f"🤖 MCP Agent 응답:\n{state['model_response']}\n\n"
    ...
    def build_graph(self):
        ...
        try:
            ...
            print("✅ MCP LangGraph Agent가 성공적으로 빌드되었습니다.")
    ...
    def get_info(self) -> dict:
        ...
        return {
            ...
            "state_schema": list(MCPAgentState.__annotations__.keys()),
            ...
            "features": ["상태 관리", "노드 기반", "MCP Tool calling", "확장 가능", "비동기 지원", "디버깅 용이", "설정 파일 기반"]
        }
    
    async def cleanup(self) -> None:
        """리소스 정리"""
        await self.mcp_manager.cleanup()
        print("🧹 MCP Agent 정리 완료")
